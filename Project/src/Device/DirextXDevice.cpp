...
unsigned long DX11Device::Init(HWND handle, unsigned long width, unsigned long height, bool windowmode) {
	...
	//this->context->OMSetRenderTargets(1, rtv, this->getDSV());
	this->context->OMSetRenderTargets(1, rtv, nullptr);
	DEBUG(ImGui_ImplDX11_Init(handle, this->device, this->context));
	DEBUG(ImGui_ImplDX11_CreateDeviceObjects());
	return ret;
}
