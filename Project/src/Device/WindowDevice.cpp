...
DEBUG(extern LRESULT ImGui_ImplDX11_WndProcHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam));
namespace GFW {


//後から修正(各Windowデバイスごとにイベント処理できるように変更)
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	DEBUG(ImGui_ImplDX11_WndProcHandler(hWnd, uMsg, wParam, lParam));
	switch (uMsg) {
	case WM_CREATE:
		break;
	case WM_CLOSE:
...
unsigned long WindowDevice::Init(HINSTANCE ins, int ncms, String title, unsigned long width, unsigned long height, bool windowmode) {
	...
	DX11Device::getInstance().Init(this->handle, this->width, this->height, this->windowmode);

	this->runflg = true;
	//this->thread = boost::thread(&GFW::WindowDevice::MessageLoop, this);
	return ret;
}
